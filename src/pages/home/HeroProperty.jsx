...
const HeroProperty = () => {
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden">
      ...
      {/* Background Images with Bright Effect */}
      ...
      {/* Bright White Overlay (instead of dark) */}
      <div className="absolute inset-0 bg-white/10"></div>

      {/* Left Arrow */}
      <button
        onClick={prevSlide}
        className="absolute left-4 top-1/2 -translate-y-1/2 bg-amber-600 text-white p-4 rounded-md z-20 hover:bg-amber-700"
      >
        <ChevronLeft size={24} />
      </button>
      ...
      <button
        onClick={nextSlide}
        className="absolute right-4 top-1/2 -translate-y-1/2 bg-amber-600 text-white p-4 rounded-md z-20 hover:bg-amber-700"
      >
      ...
      <div className="absolute right-6 md:right-16 top-1/2 -translate-y-1/2 bg-white/95 backdrop-blur-md rounded-2xl shadow-2xl p-6 md:p-10 max-w-lg z-20">
        ...
        <div className="flex items-center justify-between flex-col sm:flex-row gap-4">
          ...
          <button
            onClick={Tour}
            className="bg-amber-600 text-white px-6 py-3 rounded-lg hover:bg-amber-700 transition w-full sm:w-auto"
          >
            Schedule a Tour
          </button>
        </div>
